...
(() => {
  ...
  function getCached(url) {
    return new Promise((resolve) => {
      ...
      chrome.storage.local.get(k, (obj) => {
        ...
        const ageH = (Date.now() - hit.ts) / 36e5;
        if (ageH > settings.cacheTtlHours) return resolve(null);
        resolve(hit);
      });
    });
  }
  ...
  function setCached(url, data) {
    chrome.storage.local.set({ [cacheKey(url)]: { ...data, ts: Date.now() } }, () => {
      // vol of anderszins geweigerd: niet stilzwijgend voorbijgaan
      if (chrome.runtime.lastError) {
        log("opslaan mislukt:", chrome.runtime.lastError.message);
        pruneCache(true);
      }
    });
  }
  ...
  function pruneCache(force = false) {
    chrome.storage.local.get("prunedAt", (o) => {
      const now = Date.now();
      if (!force && o.prunedAt && now - o.prunedAt < PRUNE_EVERY_MS) return;

      chrome.storage.local.set({ prunedAt: now });
  ...
  function processLinks() {
    ...
    chrome.storage.local.get(allKeys, (store) => {
      const now = Date.now();
...
/* Wat telt níét mee voor de rating: er is niet gespeeld. Een opgave hoort
  ...
  function rememberSelf() {
    ...
    if (!url) return;
    chrome.storage.local.get("selfProfile", (o) => {
      if (o.selfProfile !== url) chrome.storage.local.set({ selfProfile: url });
    });
  }
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === "diagnose") {
      diagnose(msg.url).then(sendResponse);
      return true; // async
    }
  });

  /** Verzamelt info voor een bugrapport als iets niet klopt. */
  async function diagnose(forcedUrl) {
